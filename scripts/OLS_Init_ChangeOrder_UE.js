...
function(record, log, search, serverWidget) {
    ...
    function beforeLoad(context) {
		...
		context.form.getSublist('line').getField('amount').updateDisplayType({displayType:'hidden'});
		context.form.getSublist('line').getField('account').updateDisplayType({displayType:'hidden'});
		...
		if(context.type != 'view'){
        	...
        	if (context.request && context.request.parameters){
	            ...
	            if (!projectID || projectID == '') {
					...
					var coProjectID = context.newRecord.getValue({fieldId:'custbody_ols_co_project'});
					if (coProjectID) {
			            log.debug({
				...
				if (projectID){
					...
					var coSOID = context.newRecord.getValue({fieldId:'custbody_ols_co_so'});
	    			context.form.getField('custbody_ols_co_so').updateDisplayType({displayType:'hidden'});
	                ...
	                var searchFilters = [];
	                searchFilters.push( search.createFilter({name: 'type', operator: search.Operator.ANYOF, values: ["SalesOrd"]}));
	                searchFilters.push( search.createFilter({name: 'mainline', operator: search.Operator.IS, values: ["T"]}));
	                searchFilters.push( search.createFilter({name: 'internalidnumber', join:"jobmain" , operator: search.Operator.EQUALTO, values: projectID}));

	                var searchColumns = [];
	                searchColumns.push( search.createColumn({name: 'tranid', label: 'Sales Order Number', sort: search.Sort.ASC}));
        ...
        // disable "Status" field for "Approved" Change Orders
        	...
        	context.form.getField('custbody_ols_co_customer').updateDisplayType({displayType:'disabled'});
        	context.form.getField('custbody_ols_co_project').updateDisplayType({displayType:'disabled'});

            var coStatus = context.newRecord.getValue({fieldId:'status'});
            if (coStatus == 'Approved'){
                context.form.getField('transtatus').updateDisplayType({displayType:'disabled'}); 
            }
		...
		//      2) also for not "Loaded" items of CO ( it means newly created ), insert them into the related SO
            ...
            if (coCheckLoaded){
	            ...
	            if (coNewSO != coOldSO){
					...
					for (var soOldlineNum = soOldItemCount-1; soOldlineNum >= 0; soOldlineNum--){
						var soItemCO = soRecOld.getSublistValue({sublistId: 'item', fieldId: 'custcol_ols_so_item_co', line: soOldlineNum});
					...
					for (var coLineNum = 0 ; coLineNum < coItemCount; coLineNum++){
	            		...
	            		var coItemID = coRecNew.getSublistValue({sublistId: 'line', fieldId: 'custcol_ols_co_item_internalid', line: coLineNum});
	    				var coItemUnit = coRecNew.getSublistValue({sublistId: 'line', fieldId: 'custcol_ols_co_item_unit', line: coLineNum});
	    				var coItemQty = coRecNew.getSublistValue({sublistId: 'line', fieldId: 'custcol_ols_co_item_quantity', line: coLineNum});
	    				var coItemDesk = coRecNew.getSublistValue({sublistId: 'line', fieldId: 'custcol_ols_co_item_description', line: coLineNum});
	    				var coItemRate = coRecNew.getSublistValue({sublistId: 'line', fieldId: 'custcol_ols_co_item_unitsell', line: coLineNum});
	    				var coItemAmount = coRecNew.getSublistValue({sublistId: 'line', fieldId: 'custcol_ols_co_item_amount', line: coLineNum});
	            		// if (coItemAmount < 0) coItemAmount = coItemAmount * -1;

						soRecNew.selectNewLine({ sublistId: 'item'});

						soRecNew.setCurrentSublistValue({sublistId: 'item', fieldId: 'custcol_ols_so_item_co',value: coID});
						soRecNew.setCurrentSublistValue({sublistId: 'item', fieldId: 'item', value: coItemID});
						soRecNew.setCurrentSublistValue({sublistId: 'item', fieldId: 'custcol_ols_co_item_unit',value: coItemUnit});
						soRecNew.setCurrentSublistValue({sublistId: 'item', fieldId: 'quantity', value: coItemQty});
						soRecNew.setCurrentSublistValue({sublistId: 'item', fieldId: 'description', value: coItemDesk});
						soRecNew.setCurrentSublistValue({sublistId: 'item', fieldId: 'rate', value: coItemRate});
						soRecNew.setCurrentSublistValue({sublistId: 'item', fieldId: 'amount', value: coItemAmount});
					...
					for (var coLineNum = 0 ; coLineNum < coItemCount; coLineNum++){
						...
						var soItemSpecID = soUpdatedRec.getSublistValue({sublistId: 'item', fieldId: 'id', line: soNewItemCount + coLineNum});
			            // log.debug({
			            //     title:'Updated SO Item Specific ID',
			            //     details: soItemSpecID
			            // });

						coRecNew.setSublistValue({sublistId: 'line', fieldId: 'custcol_ols_co_item_loaded', line: coLineNum, value: true});
						coRecNew.setSublistValue({sublistId: 'line', fieldId: 'custcol_ols_co_item_so_itemid', line: coLineNum, value: soItemSpecID});
					...
					for (var coLineNum = 0 ; coLineNum < coItemCount; coLineNum++){
						...
						var coItemLoaded = coRecNew.getSublistValue({sublistId: 'line', fieldId: 'custcol_ols_co_item_loaded', line: coLineNum});
						...
						if (!coItemLoaded) {
							...
							soRecOld.setCurrentSublistValue({sublistId: 'item', fieldId: 'custcol_ols_so_item_co',value: coID});
							soRecOld.setCurrentSublistValue({sublistId: 'item', fieldId: 'item', value: coItemID});
							soRecOld.setCurrentSublistValue({sublistId: 'item', fieldId: 'custcol_ols_co_item_unit',value: coItemUnit});
							soRecOld.setCurrentSublistValue({sublistId: 'item', fieldId: 'quantity', value: coItemQty});
							soRecOld.setCurrentSublistValue({sublistId: 'item', fieldId: 'description', value: coItemDesk});
							soRecOld.setCurrentSublistValue({sublistId: 'item', fieldId: 'rate', value: coItemRate});
					...
					for (var coLineNum = 0 ; coLineNum < coItemCount; coLineNum++){
						...
						if (!coItemLoaded){
							...
							var soItemSpecID = soNewRec.getSublistValue({sublistId: 'item', fieldId: 'id', line: soOldItemCount + coUnloadedItemCount});
				            // log.debug({
					...
					for (var soLineNum = soOldItemCount-1; soLineNum >= 0; soLineNum--){

						var soItemCO = soNewRec.getSublistValue({sublistId: 'item', fieldId: 'custcol_ols_so_item_co', line: soLineNum});

						if (soItemCO == coID) {

							var coItemExitingCheck = false;

							var soItemSpecID = soNewRec.getSublistValue({sublistId: 'item', fieldId: 'id', line: soLineNum});

							for (var coLineNum = 0 ; coLineNum < coItemCount; coLineNum++){

								var coItemSpecID = coRecNew.getSublistValue({sublistId: 'line', fieldId: 'custcol_ols_co_item_so_itemid', line: coLineNum});
								...
								if (soItemSpecID == coItemSpecID){
									...
									soNewRec.setCurrentSublistValue({sublistId: 'item', fieldId: 'custcol_ols_so_item_co',value: coID});
									soNewRec.setCurrentSublistValue({sublistId: 'item', fieldId: 'item', value: coItemID});
									soNewRec.setCurrentSublistValue({sublistId: 'item', fieldId: 'custcol_ols_co_item_unit',value: coItemUnit});
									soNewRec.setCurrentSublistValue({sublistId: 'item', fieldId: 'quantity', value: coItemQty});
									soNewRec.setCurrentSublistValue({sublistId: 'item', fieldId: 'description', value: coItemDesk});
									soNewRec.setCurrentSublistValue({sublistId: 'item', fieldId: 'rate', value: coItemRate});
									soNewRec.setCurrentSublistValue({sublistId: 'item', fieldId: 'amount', value: coItemAmount});
        ...
        // before submit on delete, if it was Approved & Loaded Change Order, remove items at the related Sales Order
			...
			if (coCheckLoaded){
		        ...
		        for (var lineNum = soItemCount-1; lineNum >= 0; lineNum--){
            		var soItemCO = soRec.getSublistValue({sublistId: 'item', fieldId: 'custcol_ols_so_item_co', line: lineNum});
    ...
    function afterSubmit(context){
		...
		if(context.type != 'create' 
			&& context.newRecord.getValue({fieldId:'transtatus'}) == context.oldRecord.getValue({fieldId:'transtatus'})
			&& context.newRecord.getValue({fieldId:'custbody_ols_co_loadcheck'}) == context.oldRecord.getValue({fieldId:'custbody_ols_co_loadcheck'})
			&& context.newRecord.getValue({fieldId:'custbody_ols_co_totalamount'}) == context.oldRecord.getValue({fieldId:'custbody_ols_co_totalamount'})
		){
	        log.debug({
		...
		var projectID = context.newRecord.getValue({fieldId:'custbody_ols_co_project'});
		var projectRec = record.load({
			type: 'job',
			id: projectID
		});
